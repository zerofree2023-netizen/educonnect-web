// lib/server/parsers/sjtuDoctorCatalogGptFallback.ts

export const SJTU_DOCTOR_TABLE_HEADER = [
...
export type SjtuDoctorCatalogRow = {
  idx: number;
...
function parseSection(
  ...
  const rows: SjtuDoctorCatalogRow[] = [];
...
function cleanupRows(rows: SjtuDoctorCatalogRow[]) {
...
function fillSjtuDoctorKnownTuition(row: SjtuDoctorCatalogRow): SjtuDoctorCatalogRow {
  const next: any = { ...(row || {}) };

  // 上海交通大学 2026 国际博士研究生中文授课专业目录：
  // 博士中文授课目录学制 4 years，学费 45,500 RMB/Year
  next.duration_years = 4;
  next.tuition_rmb_per_year = 45500;
  next.tuition_total_rmb = null;
  next.tuition_is_per_year = true;
  next.tuition_note = "45,500 RMB/Year";

  return next;
}

export function parseSjtuDoctorCatalogPdfByGpt(
  rawText: string,
  ...
  if (!hasHeader(lines)) {
    return {
      ...
      rows: [],
      meta: {
        parser: "sjtu_doctor_catalog_gpt_fallback_v1",
        title: head.slice(0, 200),
        rows: 0,
        error: "header_not_matched",
        table_header: SJTU_DOCTOR_TABLE_HEADER,
      },
    };
  }
  ...
  console.log("[SJTU_DOCTOR_GPT_FALLBACK_SECTION_DEBUG]", {
    headerMatched: true,
  ...
  const finalRows = rows.map(fillSjtuDoctorKnownTuition).map((row, idx) => ({
    ...row,
    idx: idx + 1,
  }));

  console.log("[SJTU_DOCTOR_GPT_FALLBACK_ROWS_DEBUG]", {
